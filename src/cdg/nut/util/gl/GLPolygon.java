...
public class GLPolygon implements ISelectable {
	...
	private boolean autoClipping;
	private boolean clipping;
	private boolean scaleWithBoundingBox = false;
	private Vertex4 clippingArea;
	private GLColor color;
	private int bs = -1;
		
	public GLPolygon(int x, int y, int width, int height, VertexData[] data, int[] indices)
	{
		this(x,y,width,height,4,data,indices);
	}
	
	public GLPolygon(int x, int y, int width, int height, int edgeCount, VertexData[] data, int[] indices)
	{
	...
	{
		this(x,y,width,height,Colors.WHITE.getGlColor(), false);
	}
	
	public GLPolygon(int x, int y, int width, int height, GLColor col)
	{
		this(x,y,width,height,col, false);
	}
	
	public GLPolygon(int x, int y, int width, int height, GLColor col, boolean cref)
	{
		float[] tpos = Utility.pixelToGL(x, y);
		float[] tsize = Utility.pixelSizeToGLSize(width, height);
		this.x = tpos[0];
	...
	public GLPolygon(int x, int y, int width, int height, GLColor col, boolean cref, int bs)
	{
		float[] tpos = Utility.pixelToGL(x, y);
		float[] tsize = Utility.pixelSizeToGLSize(width, height);
		this.x = tpos[0];
	...
	{
		this(x,y,width,height,cref,(float)Math.min(height, width)/2.0f,(float)Math.min(height, width)/2.0f,edgeCount,Colors.WHITE.getGlColor());
	}
	
	public GLPolygon(int x, int y, int width, int height, boolean cref, float radius, int edgeCount)
	{
		this(x,y,width,height,cref,radius,radius,edgeCount,Colors.WHITE.getGlColor());
	}
	
	public GLPolygon(int x, int y, int width, int height, boolean cref, float radius, int edgeCount, GLColor col)
	{
		this(x,y,width,height,cref,radius,radius,edgeCount,col);
	}
	
	public GLPolygon(int x, int y, int width, int height, boolean cref, float xrad, float yrad, int edgeCount, GLColor col)
	{
		float[] tpos = Utility.pixelToGL(x, y);
		float[] tsize = Utility.pixelSizeToGLSize(width, height);
		this.x = tpos[0];
	...
	public GLPolygon(int x, int y, int width, int height, boolean cref, float xrad, float yrad, int edgeCount, GLColor col, int bs)
	{
		float[] tpos = Utility.pixelToGL(x, y);
		float[] tsize = Utility.pixelSizeToGLSize(width, height);
		this.x = tpos[0];
	...
	private void createQuad()
	{
		...
		this.data = Utility.generateQuadData(this.points, this.color, st);
	...
	private void createBorderQuad()
	{
		...
		{
			data[i] = new VertexData(this.points[i], this.color);
		}
	...
	private void generateCrefPoly()
	{
		...
		{
			data[i] = new VertexData(points.get(i), this.color);
		}
